...
As an open source project, HugeGraph is looking forward to be contributed from everyone, and we are also grateful to all of the contributors.

We will describe the contribution process of HugeGraph as follows:

## 1. Preparation

Before we submit the code, we need to do some preparation:
...
If you encounter a bug, please go to [GitHub Issues](https://github.com/hugegraph/hugegraph/issues) to report it and feel free to [create an issue](https://github.com/hugegraph/hugegraph/issues/new).
...
We assume that we need to modify some files like "HugeGraph.java" and "HugeFactory.java":
...
#### 3.3 Commit changes to git repo locally

After the code has been completed, we submit them to the local repo:
...
>  Please remember to fill in the issue id, which was generated after issue creation.

#### 3.4 Push commit to GitHub fork repo

Push the local commit to GitHub fork repo(https://github.com/GITHUB_USER_NAME/hugegraph):
...
We can start the code review after all the automatic checks are passed:

- Contributor License Agreement is signed
- Travis CI build passed (Automatically Test and Deploy)

The commit will be merged if there is no problem after review.
...
If we have not passed the review, don't be discouraged. Usually a commit needs to be reviewed several times! Please follow the review comments and make further changes.
